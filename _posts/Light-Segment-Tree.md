...
写线段树的时候维护节点，指针，层级是一件很讨厌的事，所以直接开成标准 $2^n$ 大小，就可以直接使用简短计算算出层级位置，而不需要使用树状结构来维护，只需要一个类 ST 表数据结构计科。
这是一份按层存储的线段树模板，使用 `st[i][j]` 和 `lazy[i][j]` 表示长度为 `2^i` 的第 `j` 个区间节点。  模板支持区间加和区间求和，区间采用左闭右开 `[l, r)` 的写法。
